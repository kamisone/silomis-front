...
export default function BlogTagsPage() {
  ...
  async function load() {
    setLoading(true);
    try {
      setTags(await api.get<Tag[]>("/next-api/admin/blog/tags"));
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (form.id) await api.patch(`/next-api/admin/blog/tags/${form.id}`, payload);
      else await api.post("/next-api/admin/blog/tags", payload);
      setForm(null);
      await load();
    } catch (err) {
  ...
  async function handleDelete(tag: Tag) {
    ...
    await api.delete(`/next-api/admin/blog/tags/${tag.id}`);
    await load();
  }
  ...
  return (
    <div className={ui.page}>
      <div className={ui.pageHeader}>
        <h1 className={ui.pageTitle}>Blog tags</h1>
